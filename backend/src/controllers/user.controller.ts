...
export const getAllAdminsController = async (req: Request, res: Response) => {
  ...
  res.status(200).json({ message: "List admin", data });
};

export const createAdminController = async (req: Request, res: Response) => {
  const data = await UserService.createAdmin(req.body);
  res.status(201).json({ message: "Admin baru berhasil dibuat", data });
};

export const updateAdminByIdController = async (
  req: Request,
  res: Response
) => {
  const { id } = req.params;
  const data = await UserService.updateAdminById(Number(id), req.body);
  res.status(200).json({ message: "Data admin diupdate", data });
};
